...
module.exports = function(content) {
	...
	for (var i = 0; i < processed.length; i++) {
		var fullLine = processed[i];
		var lineNum = i + 1;

		var vars = fullLine.match(/(?:\$)[a-z][a-zA-Z0-9]+\b/g);
		...
		if (vars) {
			A.Array.each(
				...
				function(item, index) {
					...
					fullLine = fullLine.replace(camel, processedVarName);
				}
			);
		}

		fullLine = convertAlertToState(fullLine, i, processed);

		processed[i] = fullLine;

		var varMatch = fullLine.match(/(\$)([a-zA-Z0-9_-]+\b)/g);
				...
				function(item2, index2, collection) {
					...
					if (checkProcessedVarRemoved(varName)) {
						if (REMOVE_OLD_VARS) {
							...
							fullLine = processed[i] = '/* $' + varName + ' has been removed in Bootstrap 3 -- ' + fullLine + ' */';
						}
		...
		convertPaddingVars(fullLine, i, processed);
	}
